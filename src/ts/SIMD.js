...
// in case of proto manip
const FuncRefMap = class extends WeakMap {}

{
	const proto = FuncRefMap.prototype;
...
// stored in Vector class closures, init scope near the end, and in `splat`
const funcRefs = new FuncRefMap;

// Vector.splat
const splat = function(value) {
	// throw new RuntimeError("%Vector%.splat called a non vector");

	const func = funcRefs.get(this) ?? unreachable;

	// this uses dynamicially typed param passing
	// also `this` is dynamic too...
...
// initialization block
{
	...
	for (const [ vector_class, funcRef ] of vector_classes) {
		funcRefs.set(
			vector_class,
			funcRef
		);
